...
namespace HankoSpa.Services
{
    public class ServicioService : IServicioServices
    {
        ...
        private const string CacheKey = "Servicios";
        ...
        public async Task<Response<List<ServiceDTO>>> GetAllAsync()
        {
            ...
            var cached = SimpleCacheManager.Instance.Get<List<ServiceDTO>>(CacheKey);
            if (cached != null)
            {
                return new Response<List<ServiceDTO>>(true, "Servicios obtenidos correctamente. (desde caché)", cached);
            }
